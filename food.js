var foodObj=function(){
	...
	this.timer={};   //{ "timer17-1":[定时器,], "timer5-20":[定时器,] }
	...
	if(this.num<1){
		...
		while(i<this.num){
			...
			if(board.boardArr[row][col].available){
				...
				board.boardArr[row][col].className="normal-food"
...
foodObj.prototype.generateMagicFoods=function(){
	var me=this;
	if(Math.random()<0.05 && this.magicNum<1){
		this.magicNum=Math.random()>0.1?2:1;   
		var i=0;
